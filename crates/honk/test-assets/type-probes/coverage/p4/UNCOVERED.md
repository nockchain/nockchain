...
- `runes/cram.rs` lines and outcomes that only the unit tests miss (U): `down_item` L58, L60, L76-85, B78T/F; `collect` L109; `inline_parsers` L272-273, L291-292, B213T, B272F, B272c2T/F, B272c3F; `fens` L401-409; `look` L507, L510-511, B508c2F; `cur_indent` L525, L527, L529; `cur_to_tarp` L552, L554-555; `close_par` L613, L615-622, B611T, B613F; `parse_block` L651; `open_item` L683, L688, B685F; `line` L706, L753-754, L775, B751c2T, B752T, B752c2T/F. uncovered: no unit test exercises it yet (the `sail_cram_*` regression probes reach it).
- `bar.rs` `barbuc_wide` L210, L213, L215-216, B210F (P; unit-tested): uncovered: no probe uses a wide `|$(...)` yet.
- `utils.rs` `hoon_to_noun_uncached` `%leaf` L12303-12306 (P; unit-tested): uncovered: no probe puts an obsolete `$5`-style leaf in an arm body yet.
- `buc.rs` `buctis_irregular` B626T (UP): uncovered: no test exercises it yet (`=name=spec` where the spec autonames to `%$`).
- `sail.rs` `cord_bytes` L75, B74T (UP): uncovered: no test exercises it yet (an empty cord after `:`, as in `;p:''`).
- `sail.rs` `dedent_block` B304F (UP): uncovered: no test exercises it yet (a `"""` block whose first line is empty). L311, B310T (a later line indented less than the `"""`) and B296c2F (any line indented partway to it) (UP): hoon-138 `++inde` rejects the same input; no test exercises it yet.
- `cram.rs` `collect` (heading ids) L95, L106, B94F, B99F, B100F, B101F, B104F (UP): uncovered: no test exercises it yet. They need a heading that holds a `{...}` embed, a link or image, or an embedded node shaped like a text node.
- `cram.rs` `cash` B140F and `calf` B158F (UP): uncovered: no test exercises it yet (a tab or other control character inside a `*`, `_`, `"`, `[`, `(` or backtick span).
- `cram.rs` `inline_parsers` B213F (UP): uncovered: no test exercises it yet (markdown text with a `0` not followed by a letter or digit). B272T, B272c3T (UP): uncovered: no test exercises it yet (a `++arm` reference whose name continues with a digit or `-`).
- `cram.rs` indentation errors (UP): `back` L538-540, B537T and `line` L739, B738T (an outdent into the middle of a container's indentation), `line` L745-746 (a line indented other than 0 or 8 columns past its container), `read_line` L580, B579F (a markdown line running to the end of the file with no newline) and L585, B584T (a non-space inside the inner indentation), and `line` L785-786 (either `read_line` failure while continuing a paragraph). hoon-138 `++cram` fails on the same inputs (`++back`, `columns-advanced`, `unterminated-line`, `expected-indent`); no test exercises them yet.
- `cram.rs` `read_line` trailing spaces L595-596, B594T (UP): uncovered: no test exercises it yet (a markdown line with trailing spaces).
- `cram.rs` `line` L734-735, B733T (UP): uncovered: no test exercises it yet (a paragraph that fails to parse, ended by a line that opens an item instead of a blank line).
- `cram.rs` `run` B811c2F (UP): uncovered: no test exercises it yet.
...
- `utils.rs` `spec_to_noun` `Loop` L13342-13344, `Made` L13346-13352, `BucBuc` L13375-13382, `BucDot` L13405-13412, `BucFas` L13434-13441, `BucTic` L13457-13464, `BucZap` L13482-13489 (P; unit-covered): hoon-138 has no `$$`, `$.`, `$/`, `` $` `` or `$!` rune, and `%loop` and `%made` are only built internally.
...
- `buc.rs` `buclus` L191-192, B191T/F (UP): the `matches!` guard is reported with no counts although its successor L194 runs in the unit tests; the pre-existing identical `%gist` case needs an inner spec that already carries the same postfix doc, which the parser never builds.
