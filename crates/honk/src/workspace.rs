...
use crate::build_cache::{BuildCache, CacheObjectKind, CacheRead, CacheWrite};
...
/// Artifact shape requested from the file/workspace compiler.
///
/// These variants intentionally mirror the established `honk` CLI modes. The
/// public type is transport-neutral so gRPC and a future LSP adapter can share
/// one in-process request model without sharing wire-format types.
...
pub struct WorkspaceCheckOutput {
    ...
    pub cache_invalidated: bool,
    pub cache_stats: WorkspaceCacheStats,
    /// Owned type summaries captured at existing debug spots by the
...
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct WorkspaceCompileError {
    pub diagnostic: WorkspaceDiagnostic,
}
...
impl WorkspaceCompileError {
    fn from_dyn(error: DynError) -> Self {
        let message = error.to_string();
        let mut kind = WorkspaceDiagnosticKind::Internal;
        let mut location = None;
        let mut current: Option<&(dyn Error + 'static)> = Some(error.as_ref());

        while let Some(source) = current {
            if let Some(compiler_error) = source.downcast_ref::<crate::errors::CompilerError>() {
                kind = match compiler_error.kind() {
                    Some(CompilerErrorKind::Parse) => WorkspaceDiagnosticKind::Parse,
                    Some(CompilerErrorKind::UnsupportedExpr) => {
                        WorkspaceDiagnosticKind::UnsupportedExpr
                    }
                    Some(CompilerErrorKind::Backend) => WorkspaceDiagnosticKind::Backend,
                    Some(CompilerErrorKind::Decode) => WorkspaceDiagnosticKind::Decode,
                    Some(CompilerErrorKind::Noun) => WorkspaceDiagnosticKind::Noun,
                    None => WorkspaceDiagnosticKind::Io,
                };
                location = compiler_error
                    .metadata()
                    .and_then(|metadata| metadata.location.clone());
                break;
            }
            if source.downcast_ref::<std::io::Error>().is_some() {
                kind = WorkspaceDiagnosticKind::Io;
            }
...
impl From<ArtifactMode> for CompileMode {
    fn from(mode: ArtifactMode) -> Self {
        match mode {
            ArtifactMode::Standard => Self::Standard,
            ArtifactMode::Arbitrary => Self::Arbitrary,
            ArtifactMode::Dynock => Self::Dynock,
            ArtifactMode::DynockTyped => Self::DynockTyped,
        }
    }
}
...
pub fn main_cli() {
    ...
    if raw_args.first().map(String::as_str) == Some("nockasm") {
        match crate::artifact::run_cli(&raw_args[1..]) {
            Ok(message) => {
                println!("{message}");
                return;
            }
    ...
    if raw_args.first().map(String::as_str) == Some("cache") {
        match crate::build_cache::run_cli(&raw_args[1..]) {
            Ok(message) => {
                println!("{message}");
                return;
            }
...
fn build_context_with_shared_prelude_in<'arena>(
    ...
    if env::var_os("HONK_IR_ROUNDTRIP").is_some() {
        ...
        crate::native::ir::roundtrip_check(prelude_formula, &space)?;
    ...
    if let Some(subject_ty) = subject_type_override {
        ...
        if env::var_os("HONK_IR_ROUNDTRIP").is_some() {
            crate::native::ir::type_roundtrip_check(subject_ty, &space)?;
            if env::var_os("NATIVE_HOON_TRACE").is_some() {
                ...
                if let Ok((total, distinct)) =
                    crate::native::ir::type_intern_stats(subject_ty, &space)
                {
    ...
    pub fn compile_current(
        ...
        let mut artifact = self
            ...
            .jam_product(
                ...
                request.mode.into(),
                &request.entry,
                request.directory_files.as_deref(),
            )
            .map_err(WorkspaceCompileError::from_dyn)?;
...
fn workspace_layout_fingerprint(root: &Path) -> Result<blake3::Hash> {
    ...
    let mut hasher = blake3::Hasher::new();
    hasher.update(b"honk-workspace-layout-v1\0");
    for entry in entries {
        let path = entry.path();
...
impl<'a> NativeBuildContext<'a> {
    ...
    fn enable_workspace_mode(&mut self) {
    ...
    fn workspace_files_changed(&self) -> Result<bool> {
        if !self.workspace_mode {
            return Ok(false);
        }
...
"#,
    ...
    fn compile_path(&mut self, path: &Path, need_eval: bool) -> Result<SubjectVase> {
        let canonical = self.source_canonicalize(path)?;
        let memory_content_key = match &self.workspace_sources {
        ...
        if self.workspace_mode {
            self.observed_files
                .insert(canonical.clone(), memory_content_key);
        }
    ...
    fn mint_with_sut(&mut self, sut: Noun, expr: &Hoon, vet: bool) -> Result<(Noun, Noun)> {
        ...
        if env::var_os("HONK_IR_ROUNDTRIP").is_some() {
            let space = self.ut.slab.noun_space();
            crate::native::ir::roundtrip_check(formula, &space)?;
        }
        Ok((ty, formula))
    }
