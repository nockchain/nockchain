...
use libp2p::{
    allow_block_list, connection_limits, kad, memory_connection_limits, Multiaddr, PeerId, Swarm,
};
...
mod tests {
    ...
    async fn spawn_test_node() -> (TransportHandle, PeerAddress) {
        let keypair = Keypair::generate_ed25519();
        let exclusions = PeerExclusions::default();
        let swarm = start_swarm(
            LibP2PConfig::default(),
            keypair,
            vec!["/ip4/127.0.0.1/udp/0/quic-v1"
                .parse()
        ...
        .expect("start libp2p test swarm");
        let registry = gnort::MetricsRegistry::new(gnort::RegistryConfig::default());
        let metrics = Arc::new(
            NockchainP2PMetrics::register(&registry).expect("register isolated test metrics"),
        );
        let mut handle = spawn_libp2p_transport(swarm, exclusions, metrics, Vec::new(), 32);
        let address = match next_matching(&mut handle, "libp2p listening address", |event| {
            matches!(event, TransportEvent::Listening { .. })
        })
        .await
        {
    ...
    #[tokio::test]
    async fn request_response_satisfies_transport_contract() {
        let (mut requester, _) = spawn_test_node().await;
        let (mut responder, responder_address) = spawn_test_node().await;
        let requester_id = requester.local_node_id;
        let responder_id = responder.local_node_id;
        requester
            .commands
            .send(TransportCommand::Dial {
                peer: responder_id,
                addresses: vec![responder_address],
            })
            .await
            .expect("dial responder");
        next_matching(&mut requester, "outbound connection", |event| {
            matches!(
                event,
                TransportEvent::ConnectionEstablished { peer, .. }
                    if *peer == responder_id
            )
        })
        .await;
        next_matching(&mut responder, "inbound connection", |event| {
            matches!(
                event,
                TransportEvent::ConnectionEstablished { peer, .. }
        ...
        assert_request_response_contract(&mut requester, &mut responder, responder_id).await;

        for node in [&requester, &responder] {
            node.commands
                .send(TransportCommand::Shutdown)
                .await
                .expect("shut down libp2p transport");
        }
    }
}
