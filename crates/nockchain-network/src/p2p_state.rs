...
mod tests {
    ...
    fn fresh_outbound_request_id() -> OutboundRequestId {
        ...
        OutboundRequestId::new(NEXT_REQUEST_ID.fetch_add(1, Ordering::Relaxed))
    }

    fn peer_address(address: &Multiaddr) -> PeerAddress {
        let mut ip = None;
        let mut port = None;
        for protocol in address.iter() {
            match protocol {
                ...
                libp2p::multiaddr::Protocol::Tcp(value)
                | libp2p::multiaddr::Protocol::Udp(value) => port = Some(value),
                _ => {}
            }
        }
    ...
    fn test_track_connection_records_first_ip_connection() {
        ...
        let peer_id = PeerId::random();
        let connection_id = ConnectionId::new(42 as u64);
    ...
    fn test_active_outbound_request_items_track_singletons_and_batch_items() {
        ...
        let mut state = P2PState::new(metrics, LIBP2P_CONFIG.seen_tx_clear_interval);
        let peer_id = PeerId::random();
        let single_request_id = fresh_outbound_request_id();
        let batch_request_id = fresh_outbound_request_id();
    ...
    fn test_peer_stats_snapshot_tracks_req_res_counters() {
        ...
        let peer_id = PeerId::random();
        let connection_id = ConnectionId::new(404 as u64);
    ...
    fn test_peer_stats_snapshot_tracks_connection_duration_seconds() {
        ...
        let peer_id = PeerId::random();
        let connection_id = ConnectionId::new(808 as u64);
    ...
    fn test_peer_stats_generation_stays_unknown_during_startup_until_capability_is_observed() {
        ...
        let peer_id = PeerId::random();
        let connection_id = ConnectionId::new(809 as u64);
    ...
    fn test_peer_stats_generation_tracks_capability_not_last_request_path() {
        ...
        let peer_id = PeerId::random();
        let connection_id = ConnectionId::new(810 as u64);
    ...
    fn test_reconnecting_peer_stats_reset_generation_until_capability_reobserved() {
        ...
        let peer_id = PeerId::random();
        let first_connection_id = ConnectionId::new(811 as u64);
        let second_connection_id = ConnectionId::new(812 as u64);
    ...
    fn test_lost_connection_clears_outbound_request_contexts_for_disconnected_peer() {
        ...
        let mut state = P2PState::new(metrics, LIBP2P_CONFIG.seen_tx_clear_interval);
        let peer_id = PeerId::random();
        let connection_id = ConnectionId::new(777 as u64);
        let remote_addr: Multiaddr = "/ip4/127.0.0.1/tcp/4040"
            .parse()
            .expect("valid remote addr");
    ...
    fn test_lost_connection_refreshes_req_res_inflight_metrics() {
        ...
        let peer_id = PeerId::random();
        let connection_id = ConnectionId::new(778 as u64);
        let remote_addr: Multiaddr = "/ip4/127.0.0.1/tcp/5050"
            .parse()
            .expect("valid remote addr");
    ...
    fn select_request_peers_drops_peer_with_only_excluded_ips() {
        ...
        tracker
            ...
            .entry(excluded_peer)
            .or_default()
            .insert(ConnectionId::new(1), peer_address(&excluded_addr));
        tracker
            .peer_connections
            .entry(healthy_peer)
            .or_default()
            .insert(ConnectionId::new(2), peer_address(&healthy_addr));
    ...
    fn select_request_peers_keeps_peer_with_a_clean_connection() {
        ...
        tracker
            ...
            .entry(peer)
            .or_default()
            .insert(ConnectionId::new(2), peer_address(&clean_addr));
    ...
    fn ip_bucket_request_admission_groups_ipv6_by_64() {
        ...
        let peer_a = PeerId::random();
        let peer_b = PeerId::random();
        let conn_a = ConnectionId::new(1001 as u64);
        let conn_b = ConnectionId::new(1002 as u64);
    ...
    fn ip_bucket_connection_count_groups_ipv6_by_64() {
        ...
        let peer_a = PeerId::random();
        let peer_b = PeerId::random();
        let conn_a = ConnectionId::new(1003 as u64);
        let conn_b = ConnectionId::new(1004 as u64);
    ...
    fn gossip_bucket_rejects_over_capacity_before_refill() {
        let mut state = P2PState::new(isolated_test_metrics(), 100);
        let peer = PeerId::random();
        let conn = ConnectionId::new(1005 as u64);
    ...
    fn request_peer_selection_interleaves_ip_buckets() {
        ...
        state.track_connection(
            ConnectionId::new(1101 as u64),
            peer_a,
            peer_address(&addr_a),
            ConnectionDirection::Outbound,
        );
        state.track_connection(
            ConnectionId::new(1102 as u64),
            peer_b,
            peer_address(&addr_b),
            ConnectionDirection::Outbound,
        );
        state.track_connection(
            ConnectionId::new(1103 as u64),
            peer_c,
            peer_address(&addr_c),
            ConnectionDirection::Outbound,
        );
