...
use libp2p::{allow_block_list, connection_limits, memory_connection_limits, Multiaddr};
...
pub use crate::backend::iroh::IrohPeer;
use crate::backend::iroh::{spawn_iroh_transport, IrohTransportConfig};
use crate::backend::libp2p::{spawn_libp2p_transport, start_swarm};
...
// TODO: This wire is a historical placeholder shared with other Nockchain pokes
// and should eventually be replaced without changing its kernel-facing ABI.
...
/// an empty window.
fn next_backbone_window<T: Clone>(pool: &[T], cursor: &mut usize, count: usize) -> Vec<T> {
    if pool.is_empty() {
        return Vec::new();
    }
...
fn should_redial_initial_peers<T>(
...
pub struct IrohBackendConfig {
    pub keypair: Keypair,
    pub bind: SocketAddr,
    pub advertised_addresses: Vec<PeerAddress>,
    pub initial_peers: Vec<IrohPeer>,
    pub backbone_peers: Vec<IrohPeer>,
    pub backbone_dial_count: usize,
    pub force_peers: Vec<IrohPeer>,
}
...
pub fn make_network_driver(config: NetworkDriverConfig) -> IODriverFn {
    ...
    Box::new(move |handle| {
        ...
        Box::pin(async move {
            ...
            debug!("Network config: {:?}", libp2p_config);
            ...
            let request_timeout = libp2p_config.request_response_timeout();
            let max_frame_size = libp2p_config.gen2_batch_max_bytes();
            let (
                mut transport,
                initial_peer_ids,
                backbone_peer_ids,
                ...
                TransportBackendConfig::Libp2p(Libp2pBackendConfig {
                    keypair,
                    bind,
                    allowed,
                    limits,
                    ...
                    .map_err(|error| {
                        error!(%error, "Could not create libp2p swarm");
                        NockAppError::OtherError(String::from("Could not start libp2p transport"))
                    })?;
                    let initial_peer_ids = multiaddr_peer_ids(&initial_peers);
                    let backbone_peer_ids = multiaddr_peer_ids(&backbone_peers);
                    let force_peer_ids = multiaddr_peer_ids(&force_peers);
                    let mut bootstrap_peers = initial_peers;
                    bootstrap_peers.extend(backbone_peers);
                    bootstrap_peers.extend(force_peers);
                    (
                        spawn_libp2p_transport(
                            swarm,
                            peer_exclusions.clone(),
                            metrics.clone(),
                            bootstrap_peers,
                ...
                TransportBackendConfig::Iroh(IrohBackendConfig {
                    keypair,
                    bind,
                    ...
                    let initial_peer_ids = initial_peers.iter().map(|peer| peer.node_id).collect();
                    let backbone_peer_ids =
                        backbone_peers.iter().map(|peer| peer.node_id).collect();
                    let force_peer_ids = force_peers.iter().map(|peer| peer.node_id).collect();
                    let mut bootstrap_peers = initial_peers;
                    bootstrap_peers.extend(backbone_peers);
                    bootstrap_peers.extend(force_peers);
                    let mut config = IrohTransportConfig::new(keypair, bind);
                    config.advertised_addresses = advertised_addresses;
                    config.bootstrap_peers = bootstrap_peers;
                    config.command_capacity = swarm_action_queue_capacity;
                    config.max_frame_size = max_frame_size;
                    config.request_timeout = request_timeout;
                    let transport = spawn_iroh_transport(config).await.map_err(|error| {
                        error!(%error, "Could not create Iroh endpoint");
                        NockAppError::OtherError(String::from("Could not start Iroh transport"))
                    })?;
                    (
                        transport, initial_peer_ids, backbone_peer_ids, force_peer_ids,
                        backbone_dial_count,
                    )
                }
            };
            ...
            let mut backbone_cursor: usize = if backbone_peer_ids.is_empty() {
                0
            } else {
                rand::rng().random_range(0..backbone_peer_ids.len())
            };
            if !initial_peer_ids.is_empty() {
                transport_tx
                    .send(TransportCommand::DialKnown {
                        peers: initial_peer_ids.clone(),
                    })
                    .await
                    .map_err(|_| {
            ...
            let initial_backbone_ids = next_backbone_window(
                &backbone_peer_ids, &mut backbone_cursor, backbone_dial_count,
            );
            ...
            if let Some(tx) = init_complete_tx {
                let _ = tx.send(());
                debug!("network driver initialization complete signal sent");
            }
                ...
                tokio::select! {
                    ...
                    _ = initial_peer_redial.tick() => {
                        let connected_peer_count =
                            driver_state.lock().await.peer_connections.len();
                        if should_redial_initial_peers(
                            ...
                            let mut peers = initial_peer_ids.clone();
                            peers.extend(backbone_window);
                            if !peers.is_empty() {
                                transport_tx
                    ...
                    _ = force_peer_dial.tick() => {
                        ...
                        let peers = force_peer_ids.clone();
                        if !peers.is_empty() {
                            transport_tx
                                .send(TransportCommand::DialKnown { peers })
                                .await
...
async fn handle_effect_with_dispatcher(
    ...
    match EffectType::from_noun_slab(&noun_slab) {
        ...
        EffectType::LiarBlockId => {
            ...
            // the main network select loop drains `swarm_tx`, and that same
...
pub fn iroh_socket_from_multiaddr(address: &Multiaddr) -> Result<SocketAddr, String> {
    let mut ip = None;
    let mut port = None;
    ...
    match (ip, port) {
        ...
        _ => Err(format!(
            "Iroh direct addresses require an IP and UDP port: {address}"
        )),
    }
}

pub fn iroh_peer_from_multiaddr(address: &Multiaddr) -> Result<IrohPeer, String> {
    let node_id = match address.iter().last() {
        Some(Protocol::P2p(node_id)) => node_id,
        _ => {
            return Err(format!(
                "Iroh peer address requires a trailing /p2p node ID: {address}"
            ));
        }
    };
    Ok(IrohPeer {
        node_id,
        addresses: vec![PeerAddress::new(iroh_socket_from_multiaddr(address)?)],
    })
}
