...
use crate::messages::{
    ...
    BundledTxEnvelope, EnvelopeKind, NockchainDataRequest, NockchainFact, NockchainResponse,
    ResponseEnvelope,
};
...
use crate::types::{ConnectionDirection, InboundRequestId};

pub static LIBP2P_CONFIG: LazyLock<LibP2PConfig> = LazyLock::new(LibP2PConfig::default);

#[test]
fn p2p_wire_preserves_legacy_kernel_abi() {
    let peer = PeerId::random();
    let expected_peer = peer.to_base58();

    let gossip = P2pWire::Gossip(peer).to_wire();
    assert_eq!(gossip.source, "libp2p");
    assert_eq!(gossip.version, 1);
    assert_eq!(
        gossip.tags,
        vec!["gossip".into(), "peer-id".into(), expected_peer.clone().into()]
    );

    let response = P2pWire::Response(peer).to_wire();
    assert_eq!(response.source, "libp2p");
    assert_eq!(response.version, 1);
    assert_eq!(
        response.tags,
        vec!["response".into(), "peer-id".into(), expected_peer.into()]
    );
}

#[test]
fn iroh_peer_address_requires_trailing_node_id() {
    let peer = libp2p::identity::Keypair::generate_ed25519()
        .public()
        .to_peer_id();
    let valid: Multiaddr = format!("/ip4/127.0.0.1/udp/34000/quic-v1/p2p/{peer}")
        .parse()
        .expect("valid direct peer address");
    let parsed = iroh_peer_from_multiaddr(&valid).expect("trailing peer id");
    assert_eq!(parsed.node_id, peer);
    assert_eq!(
        parsed.addresses,
        vec![PeerAddress::new("127.0.0.1:34000".parse().unwrap())]
    );

    let relayed: Multiaddr = format!("/ip4/127.0.0.1/udp/34000/quic-v1/p2p/{peer}/p2p-circuit")
        .parse()
        .expect("valid relayed multiaddr");
    let error = iroh_peer_from_multiaddr(&relayed).expect_err("identity must be trailing");
    assert!(error.contains("trailing /p2p node ID"));
}
...
static NEXT_TEST_INBOUND_REQUEST_ID: AtomicU64 = AtomicU64::new(1);
static TEST_RESPONSE_CHANNELS: LazyLock<
    StdMutex<BTreeMap<InboundRequestId, request_response::ResponseChannel<NockchainResponse>>>,
> = LazyLock::new(|| StdMutex::new(BTreeMap::new()));

fn neutral_request_id(request_id: &request_response::OutboundRequestId) -> RequestId {
    let mut hasher = DefaultHasher::new();
    request_id.hash(&mut hasher);
    RequestId::new(hasher.finish())
}

fn neutral_peer_address(address: &Multiaddr) -> PeerAddress {
    let mut ip = None;
    let mut port = None;
    for protocol in address.iter() {
        match protocol {
            libp2p::multiaddr::Protocol::Ip4(value) => ip = Some(value.into()),
            libp2p::multiaddr::Protocol::Ip6(value) => ip = Some(value.into()),
            libp2p::multiaddr::Protocol::Tcp(value) | libp2p::multiaddr::Protocol::Udp(value) => {
                port = Some(value)
            }
            _ => {}
        }
    }
    PeerAddress::new(std::net::SocketAddr::new(
        ip.expect("test address should contain an IP"),
        port.expect("test address should contain a port"),
    ))
}

fn take_test_response_channel(
    request_id: InboundRequestId,
) -> request_response::ResponseChannel<NockchainResponse> {
    TEST_RESPONSE_CHANNELS
        .lock()
        .unwrap()
        .remove(&request_id)
        .expect("response channel should be recorded for inbound request")
}
...
async fn handle_request_response(
    ...
    match message {
        request_response::Message::Request {
            ...
            let request_id =
                InboundRequestId::new(NEXT_TEST_INBOUND_REQUEST_ID.fetch_add(1, Ordering::Relaxed));
            TEST_RESPONSE_CHANNELS
                .lock()
                .unwrap()
                .insert(request_id, channel);
            gen2::handle_inbound_request(
                peer, connection_id, request, request_id, swarm_tx, equix_builder, local_peer_id,
                traffic, metrics, driver_state, req_res_limits, peer_exclusions,
            )
            .await
        }
...
fn should_redial_initial_peers_requires_seed_peers() {
    ...
    assert!(!should_redial_initial_peers::<Multiaddr>(0, &[], &[]));
...
fn next_backbone_window_handles_empty_and_small_pools() {
    let mut cursor = 0usize;
    assert!(next_backbone_window::<Multiaddr>(&[], &mut cursor, 3).is_empty());
...
async fn inflight_prefetch_does_not_suppress_kernel_singleton() {
    ...
    {
        let mut state_guard = state_arc.lock().await;
        state_guard.register_prefetch(neutral_request_id(&request_id), peer, 50, 4);
    }
                        ...
                        match event {
                            SwarmEvent::Behaviour(ReqResTestEvent::RequestResponse(
                                ...
                                let connection = ConnectionId::new(
                                    NEXT_TEST_INBOUND_REQUEST_ID.fetch_add(1, Ordering::Relaxed),
                                );
                                return (peer, connection, message);
                            }
...
fn seed_connected_peer(state: &mut P2PState, peer: PeerId, connection_seed: usize) {
    let connection_id = ConnectionId::new(connection_seed as u64);
    ...
    state.track_connection(
        connection_id,
        peer,
        neutral_peer_address(&remote_addr),
    ...
    for wave in 0..wave_count {
        ...
        run_driver_with_timeout(
            ...
            handle_request_response(
                peer,
                ConnectionId::new(900 + wave as u64),
                request_response::Message::Response {
                    request_id,
...
async fn run_block_all_miss_response_sample(
    ...
    for wave in 0..wave_count {
        ...
        run_driver_with_timeout(
            ...
            handle_request_response(
                peer,
                ConnectionId::new(1_000 + wave as u64),
                request_response::Message::Response {
                    request_id,
                    response: NockchainResponse::BatchResult {
                        results: result_items.clone(),
...
async fn run_block_batch_miss_response_sample(
    ...
    for wave in 0..wave_count {
        ...
        run_driver_with_timeout(
            ...
            handle_request_response(
                peer,
                ConnectionId::new(1_200 + wave as u64),
                request_response::Message::Response {
                    request_id,
                    response: NockchainResponse::BatchResult {
                        results: result_items.clone(),
...
async fn authenticated_outbound_gossip_conversion_records_send_metric() {
    ...
    let responder_peer_id = PeerId::random();
    ...
    let TransportCommand::SendRequest { id, peer, request } = transport_rx
        .recv()
        .await
        .expect("transport request command")
    else {
        panic!("expected transport request command");
    };
    assert_eq!(id, RequestId::new(1));
    assert_eq!(peer, responder_peer_id);
    assert!(
        driver_state
            .lock()
            .await
            .outbound_request_context(id)
            .is_some(),
        "send should retain its outbound request context"
    );
...
async fn authenticated_gossip_routes_to_kernel() {
    ...
    let response = match recv_swarm_action(&mut swarm_rx).await {
        SwarmAction::SendResponse { id, response } => {
            let channel = take_test_response_channel(id);
            responder
                .behaviour_mut()
                .request_response
...
async fn run_checkpoint_prefetch_cost_sample(
    ...
    let processing_result = tokio::time::timeout(
        ...
        handle_request_response(
            peer,
            ConnectionId::new(500 + replay_blocks as u64),
            request_response::Message::Response {
                request_id,
...
async fn run_checkpoint_requester_cost_sample(
    ...
    let processing_result = tokio::time::timeout(
        ...
        handle_request_response(
            peer,
            ConnectionId::new(400 + replay_blocks as u64),
            request_response::Message::Response {
                request_id,
                response: NockchainResponse::BatchResult { results },
            },
...
async fn run_requester_response_workload(
    ...
    run_driver_with_timeout(
        ...
        handle_request_response(
            peer,
            ConnectionId::new(90 + item_count as u64),
            request_response::Message::Response {
                request_id,
                response: NockchainResponse::BatchResult { results },
            },
...
fn test_log_outbound_failure_tracks_gen2_timeout_metrics() {
    ...
    log_outbound_failure(
        peer_id,
        neutral_request_id(&fresh_outbound_request_id()),
...
async fn wrong_peer_id_for_connected_obtained_peer_removes_stale_address_without_ip_exclusion() {
    ...
    let stale_address = PeerAddress::new("127.0.0.1:36046".parse().expect("valid socket"));
    let (transport_tx, mut transport_rx) = mpsc::channel(4);

    state_arc.lock().await.track_connection(
        ConnectionId::new(1),
        obtained,
        stale_address,
    ...
    .await
    .expect("wrong-node-id failure should be handled");

    assert!(
        !peer_exclusions.is_ip_excluded(&stale_address.socket.ip()),
    ...
    let mut pruned_peers = BTreeSet::new();
    for _ in 0..2 {
        match transport_rx.recv().await {
            Some(TransportCommand::RemoveDiscoveredAddress {
                peer: Some(peer),
                address,
            }) => {
                assert_eq!(address, stale_address);
                pruned_peers.insert(peer);
            }
            other => panic!("expected stale discovery address removal, got {other:?}"),
        }
    }
    assert_eq!(pruned_peers, BTreeSet::from([expected, obtained]));
}
...
async fn wrong_peer_id_for_unconnected_obtained_peer_records_address_cooldown() {
    ...
    let expected = PeerId::random();
    let obtained = PeerId::random();
    let stale_address = PeerAddress::new("127.0.0.1:36047".parse().expect("valid socket"));
    ...
    assert!(
        peer_exclusions.is_address_excluded(&stale_address, Some(expected)),
        "unknown wrong-node-id responses should cool down the stale address"
    );
    assert!(
        !peer_exclusions.is_ip_excluded(&stale_address.socket.ip()),
        "single wrong-node-id response should not exclude the IP before the repeat threshold"
    );
}
...
async fn req_res_driver_outbound_gen2_batch_send_updates_send_counters() {
    ...
    .await
    .expect("transport request should be queued");
    ...
    let TransportCommand::SendRequest { peer, request, .. } = transport_rx
        .recv()
        .await
        .expect("transport request command")
    else {
...
async fn req_res_driver_retry_requeues_only_retryable_and_missing_batch_items() {
    ...
    state_arc
        .lock()
        .await
        .record_outbound_request(neutral_request_id(&request_id), request_context.clone());
    ...
    assert!(
        state_arc
            .lock()
            .await
            .outbound_request_context(neutral_request_id(&request_id))
            .is_none(),
...
async fn req_res_driver_bundle_too_large_error_queues_classic_fallback() {
    ...
    state_arc
        .lock()
        .await
        .record_outbound_request(neutral_request_id(&request_id), request_context);
...
async fn req_res_driver_timeout_retries_batch_request_and_clears_context() {
    ...
    {
        let mut state_guard = state_arc.lock().await;
        state_guard.record_outbound_request(neutral_request_id(&request_id), request_context);
    }
...
async fn req_res_driver_timeout_updates_peer_stats_snapshot() {
    ...
    let peer = PeerId::random();
    let local_peer = PeerId::random();
    let connection_id = ConnectionId::new(9 as u64);
...
async fn req_res_driver_single_response_updates_peer_stats_snapshot() {
    ...
    let peer = PeerId::random();
    let local_peer = PeerId::random();
    let connection_id = ConnectionId::new(7 as u64);
...
async fn req_res_driver_block_by_height_timeout_queues_alternate_peer_retry() {
...

    {
        ...
        for (index, peer_id) in peers.iter().copied().enumerate() {
            ...
            state_guard.track_connection(
                ConnectionId::new(400 + index as u64),
                peer_id,
                neutral_peer_address(&remote_addr),
                ConnectionDirection::Inbound,
            );
...
async fn req_res_driver_batch_block_timeout_queues_alternate_peer_retry() {
...

    {
        ...
        for (index, peer_id) in peers.iter().copied().enumerate() {
            ...
            state_guard.track_connection(
                ConnectionId::new(500 + index as u64),
                peer_id,
                neutral_peer_address(&remote_addr),
                ConnectionDirection::Inbound,
            );
...
async fn req_res_driver_batch_block_not_found_queues_alternate_peer_retry() {
    ...
    let local_peer = PeerId::random();
    let connection_id = ConnectionId::new(22 as u64);
    let peers = (0..10).map(|_| PeerId::random()).collect::<Vec<_>>();
    let mut canonical_peers = peers.clone();
    canonical_peers.sort_unstable_by_key(|peer| peer.to_base58());
...

    {
        ...
        for (index, peer_id) in peers.iter().copied().enumerate() {
            ...
            state_guard.track_connection(
                ConnectionId::new(200 + index as u64),
                peer_id,
                neutral_peer_address(&remote_addr),
                ConnectionDirection::Inbound,
            );
...
async fn req_res_driver_batch_block_not_found_recycles_attempts_when_no_alternate_peer_exists() {
    ...
    let local_peer = PeerId::random();
    let connection_id = ConnectionId::new(122 as u64);
    let source_peer = PeerId::random();
    let block_request_message = ByteBuf::from(jam_block_by_height_request(10030));
    let request_id = fresh_outbound_request_id();
