    ...
    if let Ok(value) = env::var("AI_POW_CUDA_GENCODES") {
        let gencodes = value
            ...
            .map(|value| {
                let (arch, code) = value
                    .split_once(':')
                    .unwrap_or_else(|| panic!("invalid CUDA gencode {value:?}; expected ARCH:CODE"));
                assert!(
...
fn main() {
    ...
    println!("cargo:rerun-if-changed=csrc/ai_pow_v3_gemma4.cu");
    println!("cargo:rerun-if-changed=csrc/ai_pow_gemm.h");
    println!("cargo:rerun-if-changed=csrc/ai_pow_v3_peak.h");
    println!("cargo:rerun-if-changed=csrc/ai_pow_v3_gemma4.h");
