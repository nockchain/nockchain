...
- **Implementation:** Honk advertises document symbols, hover, definition, completion, references, and rename. An editor-only parsed side table owns lexical scope and structural symbol identity; compiler side tables remain authoritative for inferred types and resolved core arms/gates. Structural definition and completion traverse the native import graph breadth-first and decline ambiguous names. Open-document overlays shadow disk. References preserve exact same-document lexical identity and use a generation-stamped, declaration-keyed compiler fact index for resolved arms and gates across the latest successfully checked import graph. Rename remains restricted to lexical faces and rejects capture or collision.
- **Impact:** Local faces support definition, references, rename, and completion. Compiler-resolved imported arms and gates support cross-file references. Unambiguous local, imported, and standard-library arms and molds support definition and completion, and rune tokens navigate to their canonical prelude implementation. Molds and standard-library symbols that resolve only through the structural fallback do not yet have workspace-wide references; unrelated files outside the checked import graph are not searched. Imported or standard-library declarations cannot be renamed. Formatting, semantic tokens, and pull diagnostics remain unadvertised.
- **Resolution:** INVESTIGATING; extend owned compiler provenance to structural molds and prelude declarations, then use it for complete workspace rename without changing the compiler AST or ordinary CLI/batch results.
- **Tests affected:** structural mold/standard-library references and workspace symbol rename, plus unadvertised optional feature requests.
