...
| **D4/S1** Hoon↔Rust consensus wiring | 🟢 **Stage 1 + Stage 2 DONE + RUNTIME-VALIDATED (2026-07-14)** — `ai-pow-jets` verify jet (`[artifact commit target]` sample, canonical-A/B re-derivation, tag dispatch, boot-injected setup). Jet lives in `/common/pow` under `~% %pow-lib ..ut ~` (NOT a %dumb-inner door arm — `fort` mold fixes it to load/peek/poke; NOT a `\|^`-nested arm — those fail cold registration). `+check-pow` `%ai-pow` branch calls `ai-pow-verify:mine`; `+validate-page-without-txs` DEFERS %ai-pow to check-pow (was fail-closed). HotEntry `[one two tri qua pen pow-lib ai-pow-verify]`@1, registered in both binaries via `produce_prover_hot_state`. **Also fixed** the pre-existing `+compute-digest` crash on %ai-pow (`hashable-digest` `leaf+(jam)` → belt-safe `hash+(hash-belts-list (rip 5 (jam)))`). **Validated:** roswell `test-ai-pow-verify-jet-fires` (jet fires, %.n) + `test-ai-pow-block-rejected` (garbage %ai-pow block: build→validate-page→check-pow→jet→`%liar-block-id %failed-pow-check`); full dumb regression 262 OK. Commits 6b43891f, abbd024e. **Residual — valid-cert ACCEPTANCE:** boot-inject SETUP + pre-computed canonical cert fixture + pin production puzzle shape (protocol decision) to accept a REAL %ai-pow block (jet→%.y). |
