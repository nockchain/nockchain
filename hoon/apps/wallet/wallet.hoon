...
++  poke
  ...
  ++  do-list-notes-by-address-csv
    ...
    =/  matching-notes=(list [name=nname:transact note=nnote:transact])
      ...
      ==
    =/  csv-header=tape
      "version,name_first,name_last,assets,block_height,source_hash,required_signatures"
    =/  csv-rows=(list tape)
      %+  turn  matching-notes
      |=  [name=nname:transact note=nnote:transact]
      ?^  -.note
        ...
        """
        {(ui-to-tape:utils version)},{(trip first.name-b58)},{(trip last.name-b58)},{(ui-to-tape:utils assets.note)},{(ui-to-tape:utils origin-page.note)},{(trip source-hash-b58)},{(ui-to-tape m.lock.note)}
        """
      ::  v1 note
      =+  version=1
      =/  name-b58=[first=@t last=@t]  (to-b58:nname:transact name)
