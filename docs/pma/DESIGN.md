...
Snapshot creation runs full verification before inserting the snapshot as ready in SQLite. Snapshot restore runs full verification before copying the snapshot back over the operative PMA. Full verification validates the saved root pointer, validates the cold offset, and traverses the noun graph. The optional `structure_blake3` field can pin a structural hash, but current production snapshot creation stores `None` there; the normal cryptographic integrity check is the used-prefix BLAKE3 hash, with full traversal providing structural sanity checks.

Snapshot creation and restore preserve sparse ranges and omit writes for zero blocks. The copy preserves every byte and the logical file length, including the PMA trailer at EOF. The file length follows current PMA capacity, independently of the larger virtual reservation. Filesystems without extent discovery use a buffered copy that skips zero blocks.

Epoch compaction uses the same cumulative accepted-event processing time as snapshot rotation. `--epoch-compaction-interval-event-time` defaults to 1800 seconds and must be strictly greater than the rotation interval (900 seconds by default). `none` or `0` disables compaction; disabling rotation also disables compaction. Once due, compaction requires at least two ready rotating snapshots and copies the older retained snapshot into a new immutable epoch generation. A single long event, failed rotation, or clock adjustment cannot move the cut beyond an existing retained snapshot.

The new epoch is fully verified and synced before one SQLite transaction retires the preceding epoch, publishes its replacement, records the replay floor and current compaction head, and deletes events through the floor. The cut must be strictly below the accepted head, so a nonempty event tail remains authoritative and both rotating snapshots remain replayable. Restart reconstructs the compaction timer from events after the saved compaction head, not from the older epoch boundary. Recovery ignores bases below the floor and refuses fresh replay after pruning.

Retired artifacts are deleted after publication, with startup retrying interrupted cleanup. Physical SQLite reclamation uses `VACUUM` followed by a truncating WAL checkpoint; a persisted flag allows retry if maintenance fails. These operations run synchronously between accepted events and can pause processing. Initial reclamation of a large existing log requires temporary disk headroom for the deletion WAL and database rewrite. The old and new epochs coexist until publication and cleanup complete.
